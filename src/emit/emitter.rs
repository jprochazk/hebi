...
impl<'src> Emitter<'src> {
  ...
  fn const_name(&mut self, str: &str) -> u32 {
    self
      .state
      .builder
      .constant(Constant::Str(self.ctx.alloc_interned_string(str)))
  }
...
mod stmt {
  use super::*;
  use crate::value::object::{class, module};

  impl<'src> Emitter<'src> {
    pub fn emit_stmt(&mut self, stmt: &'src ast::Stmt<'src>) -> Result<()> {
      match stmt.deref() {
...
mod expr {
  ...
  impl<'src> Emitter<'src> {
    ...
    pub fn emit_expr(&mut self, expr: &'src ast::Expr<'src>) -> Result<()> {
      match expr.deref() {
