...
fn disasm_op(
  ...
  let written = {
    match *op {
      ...
      Op::StoreGlobal { src, name } =>            w!(f, "  sg    {src}, {name}"),
      Op::LoadField { dst, name } =>              w!(f, "  ln    {name}, {dst}"),
      Op::LoadFieldOpt { dst, name } =>           w!(f, "  ln?   {name}, {dst}"),
      Op::StoreField { obj, name, src } =>        w!(f, "  sn    {src}, {obj}, {name}"),
      Op::LoadIndex { obj, key, dst } =>          w!(f, "  li    {obj}, {key}, {dst}"),
      Op::LoadIndexOpt { obj, key, dst } =>       w!(f, "  li?   {obj}, {key}, {dst}"),
      Op::StoreIndex { obj, key, src } =>         w!(f, "  si    {src}, {obj}, {key}"),
